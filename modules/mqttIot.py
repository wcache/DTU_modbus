...
class MqttIot(CloudObservable):
    ...
    def __get_upgrade_plan_topic(self):
        cloud_config = settings.current_settings.get("mqtt_private_cloud_config")
        upgrade_plan_topic = cloud_config["upgrade_plan_topic"]
        return upgrade_plan_topic

    def __subscribe_topic(self):
        # 订阅ota升级计划主题
        upgrade_plan_topic = self.__get_upgrade_plan_topic()
        if upgrade_plan_topic:
            if self.__mqtt.subscribe(upgrade_plan_topic, qos=0) == -1:
                log.error("Topic [%s] Subscribe Failed." % self.upgrade_plan_topic)
            else:
                log.info("Topic [%s] Subscribe Successfully." % self.upgrade_plan_topic)
    ...
    def __sub_cb(self, topic, data):
        ...
        try:
            if topic == self.__get_upgrade_plan_topic():
    ...
    def post_data(self, data):
        raise NotImplementedError('self.post_data not implemented.')

    def ota_request(self):
        raise NotImplementedError('self.ota_request not implemented.')

    def ota_action(self, action, module=None):
        raise NotImplementedError('self.ota_action not implemented.')

    def device_report(self):
        raise NotImplementedError('self.device_report not implemented.')
